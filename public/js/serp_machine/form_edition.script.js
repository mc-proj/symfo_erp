$(document).ready(function() {
    ...
    $("#btn-edition-machine").on("click", function() {

        loader(true);
        let form = $("[name=form-edition-machine]");
        let form_data = form.serializeObject();
        $.ajax({
            type: "POST",
            url: route_post_edite_machine,
            dataType: "json",
            data: form_data,
            success: function(response) {
                ...
                if(typeof(erreurs) != "object") {
                    ...
                    let id = erreurs;
                    $('*[data-id="' + id + '"]').parent().parent().children().eq(0).text($("#serp_machine_nom").val());
